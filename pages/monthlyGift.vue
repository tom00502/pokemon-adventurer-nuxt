...
import _hiringPool from '@/assets/json/hiringPool.json'
...
const monthlyGift = computed(() => {
    return _monthlyGiftData.map((gift) => {
        return {
            ...gift,
            p: gift.p.map((pokeId) => pokedexStore.pokedex[pokeId] || {}),
        }
    })
})
const hiringPool = computed(() => {
    return _hiringPool.map((gift) => {
        return {
            ...gift,
            p: gift.p.map((pokeId) => pokedexStore.pokedex[pokeId] || {}),
        }
...
<template>
    <main>
        ...
        <div v-for="gift in monthlyGift" :key="gift.s">
            <div class="area">
                <span>{{ gift.s }}</span>
            </div>
            <div class="poke-list">
                <div v-for="poke in gift.p" :key="poke" :class="poke.quality">
                    {{ poke.name }}
                </div>
            </div>
        </div>
        <div v-for="gift in hiringPool" :key="gift.s">
            <div class="area">
                <span>{{ gift.s }}</span>
            </div>
            <div class="poke-list">
