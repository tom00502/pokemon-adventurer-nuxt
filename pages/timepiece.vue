...
<main>
    ...
    <table class="w-full text-center text-sm text-gray-500">
        <thead class="bg-gray-50 uppercase text-gray-700">
            <tr>
                <th scope="col" class="sticky left-0 z-10 whitespace-nowrap bg-gray-50 py-3 px-2">
                    {{ $t('timepiece.event') }}
                </th>
        ...
        <tbody>
            <tr class="border-b bg-white hover:bg-gray-50">
                ...
                <td class="whitespace-nowrap">
                    <div v-if="isGameTimeDay" class="bg-green-200 m-1  rounded-md py-1 px-2">{{ $t('timepiece.day') }}({{
                        convertRealTimeToGameTime.format('HH:mm') }})</div>
                    <div v-else class="bg-gray-500 text-white m-1  rounded-md py-1 px-2">{{ $t('timepiece.night') }}({{
                        convertRealTimeToGameTime.format('HH:mm') }})</div>
                </td>
                <td class="whitespace-nowrap">
                    <div v-if="isGameTimeDay" class="m-1  rounded-md py-1 px-2">{{ $t('timepiece.toNight', {
                        time:
                        dayNightCountdown}) }}</div>
                    <div v-else class="m-1 rounded-md py-1 px-2">{{ $t('timepiece.toDay', { time: dayNightCountdown }) }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div>{{ $t('timepiece.day') }}:
                        {{ timeConvert(3) }}-{{ timeConvert(6) }}、{{ timeConvert(9) }}-{{ timeConvert(12) }}、{{ timeConvert(15) }}-{{ timeConvert(18) }}、{{ timeConvert(21) }}-{{ timeConvert(24) }}
                    </div>
                    <div>{{ $t('timepiece.night') }}:
                        {{ timeConvert(0) }}-{{ timeConvert(3) }}、{{ timeConvert(6) }}-{{ timeConvert(9) }}、{{ timeConvert(12) }}-{{ timeConvert(15) }}、{{ timeConvert(18) }}-{{ timeConvert(21) }}
                    </div>
                </td>
            </tr>
            <tr class="border-b bg-white hover:bg-gray-50">
                ...
                <td class="whitespace-nowrap">
                    <div v-if="isSafariOpen" class="bg-green-200 m-1  rounded-md py-1 px-2">{{ $t('timepiece.open') }}
                    </div>
                    <div v-else class="bg-gray-500 text-white m-1  rounded-md py-1 px-2">{{ $t('timepiece.close') }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div v-if="isSafariOpen" class="m-1  rounded-md py-1 px-2">{{ $t('timepiece.closeIn', {
                        time:
                        safariCountdown}) }}</div>
                    <div v-else class="m-1 rounded-md py-1 px-2">{{ $t('timepiece.openIn', { time: safariCountdown }) }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div>{{ $t('timepiece.open') }}:
                        {{ timeConvert(10) }}-{{ timeConvert(12) }}、{{ timeConvert(18) }}-{{ timeConvert(22) }}</div>
                </td>
            </tr>
            <tr class="border-b bg-white hover:bg-gray-50">
                <td scope="row"
                ...
                <td class="whitespace-nowrap">
                    <div v-if="isDelicacyOpen" class="bg-green-200 m-1  rounded-md py-1 px-2">{{ $t('timepiece.open') }}
                    </div>
                    <div v-else class="bg-gray-500 text-white m-1  rounded-md py-1 px-2">{{ $t('timepiece.close') }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div v-if="isDelicacyOpen" class="m-1  rounded-md py-1 px-2">{{ $t('timepiece.closeIn', {
                        time:
                        delicacyCountdown}) }}</div>
                    <div v-else class="m-1 rounded-md py-1 px-2">{{ $t('timepiece.openIn', { time: delicacyCountdown }) }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
            ...
            <tr class="border-b bg-white hover:bg-gray-50">
                ...
                <td class="whitespace-nowrap">
                    <div v-if="isPokemenQaOpen" class="bg-green-200 m-1  rounded-md py-1 px-2">{{ $t('timepiece.open') }}
                    </div>
                    <div v-else class="bg-gray-500 text-white m-1  rounded-md py-1 px-2">{{ $t('timepiece.close') }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div v-if="isPokemenQaOpen" class="m-1  rounded-md py-1 px-2">{{ $t('timepiece.closeIn', {
                        time:
                        pokemenQaCountdown}) }}</div>
                    <div v-else class="m-1 rounded-md py-1 px-2">{{ $t('timepiece.openIn', { time: pokemenQaCountdown })
                        }}</div>
                </td>
                <td class="whitespace-nowrap">
                    <div>{{ $t('timepiece.open') }}: {{ timeConvert(12) }}-{{ timeConvert(13) }}</div>
                    <div class="text-blue-600">
                        <RouterLink :to="localePath('/pokoQA')" class="flex justify-center">
                            {{ $t('qa.title') }}
                        </RouterLink>
                    </div>
                </td>
            ...
            <tr class="border-b bg-white hover:bg-gray-50">
                ...
                <td class="whitespace-nowrap">
                    <div v-if="isBattleOpen" class="bg-green-200 m-1  rounded-md py-1 px-2">{{ $t('timepiece.open') }}
                    </div>
                    <div v-else class="bg-gray-500 text-white m-1  rounded-md py-1 px-2">{{ $t('timepiece.close') }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div v-if="isBattleOpen" class="m-1  rounded-md py-1 px-2">{{ $t('timepiece.closeIn', {
                        time:
                        battleCountdown})
                        }}</div>
                    <div v-else class="m-1 rounded-md py-1 px-2">{{ $t('timepiece.openIn', { time: battleCountdown }) }}
                    </div>
                </td>
                <td class="whitespace-nowrap">
                    <div>{{ $t('timepiece.open') }}:
                        {{ timeConvert(13) }}-{{ timeConvert(14) }}、{{ timeConvert(20) }}-{{ timeConvert(23) }}
                    </div>
                </td>
            </tr>
        </tbody>
